...
class FilterSchema(Schema):
    ...
    quality = LowerCase(
        fields.Str(
            required=True,
            validate=v.OneOf(
                ['eq', 'neq', 'match', 'nmatch', 'date-eq', 'date-lt', 'date-gt']
            ),
        )
    )
...
class FilterListSchema(Schema):
    ...
    search_type = LowerCase(
        fields.Str(validate=v.OneOf(['and', 'or'])), load_default=None
    )
...
class ListSchema(FilterListSchema):
    ...
    sort_order = LowerCase(
        fields.Str(validate=v.OneOf(['asc', 'desc'])), load_default=None
    )
