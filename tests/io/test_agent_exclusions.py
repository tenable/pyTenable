...
from tenable.errors import ForbiddenError, NotFoundError, UnexpectedValueError
...
def agentexclusion(request, api):
    ...
    """
    excl = api.agent_exclusions.create(
        str(uuid.uuid4()),
        start_time=dtime.now(timezone.utc),
        end_time=dtime.now(timezone.utc) + timedelta(hours=1),
    )
...
def test_agentexclusions_create_endtime_typerror(api):
    ...
    with pytest.raises(TypeError):
        api.agent_exclusions.create(
            str(uuid.uuid4()), start_time=dtime.now(timezone.utc), end_time='nope'
        )
...
def test_agentexclusions_create_onetime_exclusion(api):
    ...
    """
    resp = api.agent_exclusions.create(
        str(uuid.uuid4()),
        start_time=dtime.now(timezone.utc),
        end_time=dtime.now(timezone.utc) + timedelta(hours=1),
    )
...
def test_agentexclusions_create_enabled_false_exclusion(api):
    ...
    """
    resp = api.agent_exclusions.create(str(uuid.uuid4()), enabled=False)
    assert isinstance(resp, dict)
    check(resp, 'id', int)
    check(resp, 'name', str)
    check(resp, 'description', str, allow_none=True)
...
def test_agentexclusions_create_standard_users_cant_create(stdapi):
    ...
    with pytest.raises(ForbiddenError):
        stdapi.agent_exclusions.create(
            str(uuid.uuid4()),
            start_time=dtime.now(timezone.utc),
            end_time=dtime.now(timezone.utc) + timedelta(hours=1),
        )
...
def test_agentexclusions_edit_weekdays_typerror(api, agentexclusion):
    ...
    with pytest.raises(TypeError):
        api.agent_exclusions.edit(
            agentexclusion['id'], frequency='weekly', weekdays='nope'
        )
...
def test_agentexclusions_edit_weekdays_unexpectedvalue(api, agentexclusion):
    ...
    with pytest.raises(UnexpectedValueError):
        api.agent_exclusions.edit(
            agentexclusion['id'], frequency='weekly', weekdays=['MO', 'WE', 'nope']
        )
...
def test_agentexclusions_edit_dayofmonth_typerror(api, agentexclusion):
    ...
    with pytest.raises(TypeError):
        api.agent_exclusions.edit(
            agentexclusion['id'], frequency='monthly', day_of_month='nope'
        )
...
def test_agentexclusions_edit_dayofmonth_unexpectedvalue(api, agentexclusion):
    ...
    with pytest.raises(UnexpectedValueError):
        api.agent_exclusions.edit(
            agentexclusion['id'], frequency='monthly', day_of_month=0
        )
...
def test_agentexclusions_edit_freq_onetime_to_daily(api, agentexclusion):
    ...
    """
    resp = api.agent_exclusions.edit(
        agentexclusion['id'], name=str(uuid.uuid4()), frequency='daily', interval=2
    )
...
def test_agentexclusions_edit_onetime_to_weekly_valdefault(api, agentexclusion):
    ...
    """
    resp = api.agent_exclusions.edit(agentexclusion['id'], frequency='Weekly')
    check(resp, 'id', int)
    check(resp, 'name', str)
    check(resp, 'description', str, allow_none=True)
    check(resp, 'last_modification_date', int)
...
def test_agentexclusions_edit_onetime_to_weekly_valassigned(api, agentexclusion):
    ...
    """
    resp = api.agent_exclusions.edit(
        agentexclusion['id'], frequency='Weekly', weekdays=['MO', 'TU']
    )
    check(resp, 'id', int)
    check(resp, 'name', str)
    check(resp, 'description', str, allow_none=True)
...
def test_agentexclusions_edit_onetime_to_weekly_valavailable(api):
    ...
    """
    agentexclusion = api.agent_exclusions.create(
        str(uuid.uuid4()),
        frequency='Weekly',
        weekdays=['TU', 'WE'],
        start_time=dtime.now(timezone.utc),
...
def test_agentexclusions_edit_freq_onetime_to_monthly_valddefault(api, agentexclusion):
    ...
    """
    resp = api.agent_exclusions.edit(
        agentexclusion['id'], name=str(uuid.uuid4()), frequency='monthly', interval=2
    )
    assert isinstance(resp, dict)
    check(resp, 'description', str, allow_none=True)
    check(resp, 'id', int)
...
def test_agentexclusions_edit_freq_onetime_to_monthly_valavailable(api):
    ...
    resp = api.agent_exclusions.edit(
        agentexclusion['id'], frequency='monthly', interval=2
    )
    assert isinstance(resp, dict)
    check(resp, 'description', str, allow_none=True)
    check(resp, 'id', int)
...
def test_agentexclusions_edit_freq_onetime_to_yearly(api, agentexclusion):
    ...
    """
    resp = api.agent_exclusions.edit(
        agentexclusion['id'], name=str(uuid.uuid4()), frequency='yearly', interval=2
    )
    assert isinstance(resp, dict)
    check(resp, 'description', str, allow_none=True)
    check(resp, 'id', int)
