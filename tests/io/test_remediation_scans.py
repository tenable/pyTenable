...
@pytest.mark.vcr()
def test_remedy_scanned_list(api, remedy_scan):
    '''
    ...
    for remediation_scan in scan_list:
        ...
        check(remediation_scan, 'wizard_uuid', 'scanner-uuid')
...
@pytest.mark.vcr()
def test_remedyscan_status(api, remedy_scan):
    '''
    test to check scan status
    '''
    status = api.scans.status(remedy_scan['id'])
    single(status, str)


@pytest.mark.vcr()
def test_remedyscan_enable_selected_plugins(request, api):
    '''
    ...
    assert isinstance(resp, dict)
    check(resp, 'agent_scan_launch_type', str, allow_none=True)
    check(resp, 'auto_routed', int)
    check(resp, 'creation_date', int)
    check(resp, 'container_id', 'uuid')
    ...
    check(resp, 'name', str)
    check(resp, 'notification_filters', str, allow_none=True)
    ...
    check(resp, 'tag_type', 'str', allow_none=True)
    check(resp, 'target_network_uuid', 'uuid', allow_none=True)
    check(resp, 'triggers', 'str', allow_none=True)
