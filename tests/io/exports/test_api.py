...
def test_status(tvm):
    ...
    status = tvm.exports.status('vulns', '01234567-89ab-cdef-0123-4567890abcde')
...
def test_cancel(tvm):
    ...
    assert 'CANCELLED' == tvm.exports.cancel(
        'vulns', '01234567-89ab-cdef-0123-4567890abcde'
    )
...
def test_download_chunk(tvm):
    ...
    resp = tvm.exports.download_chunk(
        'vulns', '01234567-89ab-cdef-0123-4567890abcde', 1
    )
...
def export_request():
    with responses.RequestsMock() as rsps:
        url = re.compile(r'https://nourl/(vulns|assets|compliance)/export')
        rsps.add(
            responses.POST,
            url,
...
def test_base_export(export_request, tvm):
    asset_export = tvm.exports._export(export_type='assets', payload={})
    assert isinstance(asset_export, ExportsIterator)
    asset_export = tvm.exports._export(export_type='assets', payload={}, iterator=None)
