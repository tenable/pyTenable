...
from tenable.errors import BadRequestError, UnexpectedValueError
...
def test_network_create_assets_ttl_days_typeerror(api):
    with pytest.raises(TypeError):
        api.networks.create(
            'New Name - {}'.format(uuid.uuid4()), 'something', 'something'
        )
...
def test_network_edit_assets_ttl_days_invalid_input_error(api):
    with pytest.raises(BadRequestError):
        network = api.networks.create('Network-{}'.format(uuid.uuid4()))
        api.networks.edit(
            network_id=network['uuid'], name='something', assets_ttl_days=-5
        )
