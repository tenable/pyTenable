...
def fixture_custom_tagvalue(request, api, user, tagfilters):
    ...
    tag = api.tags.create(
        ...
        current_domain_permissions=[
            (user['uuid'], user['username'], 'user', ['CAN_EDIT'])
        ],
        filters=tagfilters,
    )
...
def test_tags_permission_constructor_tuple_pass(api):
    ...
    assert getattr(api.tags, '_permission_constructor')([(user, 'test', 'user')]) == [
...
def test_tags_tag_value_constructor_filter_type_unexpectedvalueerror(
    api, tagfilters, filterdefs
):
    """
    ...
    with pytest.raises(UnexpectedValueError):
        getattr(api.tags, '_tag_value_constructor')(
            filters=tagfilters, filterdefs=filterdefs, filter_type='nope'
        )
...
def test_tags_tag_value_constructor_pass(api, tagfilters, filterdefs):
    ...
    assert getattr(api.tags, '_tag_value_constructor')(
        ...
        'asset': {
            'and': [{'field': 'ipv4', 'operator': 'eq', 'value': '192.168.0.0/24'}]
        }
    }
...
def test_tags_create_value_category_description_typeerror(api):
    ...
    with pytest.raises(TypeError):
        api.tags.create(
            'a7b7ebf6-8aaf-4509-a5b3-872b7647fa86', '', category_description=1
        )
...
@pytest.mark.vcr()
def test_tags_create_filters_and_access_control_success(
    api, tagfilters, user, custom_tagvalue
):
    """
...
@pytest.mark.vcr()
def test_tags_edit_filters_and_access_control_success(
    api, user, tagfilters, custom_tagvalue
):
    """
...
def test_tags_list_constructor_filter_success(api):
    ...
    """
    resp = getattr(api.tags, '_tag_list_constructor')(
        [('value', 'eq', 'Test')], getattr(api.tags, '_filterset_tags'), None, None
    )
