...
def tags_properties_response() -> dict[str, list[dict]]:
    return {"data": [{"key": "tag_id", "readable_name": "Tag ID",
...
def test_properties_list(tenable_one_api, tags_properties_response):
    ...
    # Assert
    assert tags_properties_result == Properties(**tags_properties_response).data
...
def test_list(tenable_one_api, tags_response):
    ...
    responses.add(
        responses.POST,
        'https://cloud.tenable.com/api/v1/t1/tags/search',
        json=tags_response,
        match=[
