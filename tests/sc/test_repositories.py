...
def repository(request, vcr, admin):
    ...
    with vcr.use_cassette('repository'):
        repo = admin.repositories.create(name='example_repo')
...
def test_repositories_create_success_data_format_ipv6(admin):
    ...
    '''
    repo = admin.repositories.create(name='example_remote_repo', dataFormat='IPv6', type='remote')
    assert isinstance(repo, dict)
    check(repo, 'name', str)
    check(repo, 'id', str)
    admin.repositories.delete(int(repo['id']))
...
def test_repositories_create_success_data_format_mobile(admin):
    ...
    '''
    repo = admin.repositories.create(name='example_mobile_repo', dataFormat='mobile')
    assert isinstance(repo, dict)
    check(repo, 'name', str)
    check(repo, 'id', str)
    admin.repositories.delete(int(repo['id']))
